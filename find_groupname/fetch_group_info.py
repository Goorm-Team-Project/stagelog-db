...
def fetch_groupname_info(event_list):
    ...
    prompt = (
        ...
        "**출연진들이 하나의 'K-POP 아이돌 그룹' 멤버들로 구성된 경우**에만 해당 그룹명을 추출하세요.\n\n"
        "[필수 규칙]\n"
        "1. **1:1 대응 원칙**: 입력된 **모든 ID**에 대해 반드시 응답해야 합니다.\n"
        "2. **아이돌 식별**: 출연진이 특정 'K-POP 아이돌 그룹' 멤버들로 구성된 경우만 그룹명을 한글로 적으세요.\n"
        "   - 예: 공연명 'LE SSERAFIM TOUR', 출연 '김채원, 사쿠라, 허윤진' -> '르세라핌'\n"
        "3. **엄격한 제외**: 다음의 경우 반드시 `null`을 반환하세요.\n"
        "   - 포크, 트로트, 힙합 크루, 발라드 가수 (예: 쎄시봉, 송창식)\n"
        "   - 뮤지컬, 연극, 클래식 (예: 조승우, 옥주현)\n"
        "   - 페스티벌 (여러 가수가 따로 출연)\n"
        "4. **언어**: 그룹명은 반드시 **한국어**로 반환하세요. (BTS -> 방탄소년단, ITZY -> 있지)\n"
        "5. **형식**: `{'ID_0': '그룹명', 'ID_1': null, ...}` 형태의 JSON.\n\n"
        f"[분석할 데이터]\n{items_text}"
    )
