...
'''
fetch_stage_name_from_ai함수 Fix Note
- OpenAI API를 너무 빠르게 자주 조회해서 429에러 발생
   -> 2, 4초 늘려가며 대기했다가 다시 시도
'''
# AI에 활동명 물어보기
        ...
        try:
            ...
            if "Rate limit" in str(e) or "429" in str(e):
                if attempt < max_retries - 1:
                    ...
                    print(f"⚠️ Rate Limit 도달! [ {raw_name} ] 처리 중, [ {sleep_time:.2f}초 ] 대기 후 재시도...")
                    time.sleep(sleep_time)
                    continue
                else:
                    print(f"❌ 재시도 횟수 초과 ({raw_name}) : {e}")
                    return raw_name
            else:
...
'''
process_artist_names함수 Fix Note
- memory_cache 인자 추가 : 이번 실행 동안 알게 된 이름을 기억
'''
def process_artist_names(raw_artist_string, title, db_manager, memory_cache):
    ...
    for raw_name in raw_name_list:   
        if raw_name in memory_cache:
            final_names_list.append(memory_cache[raw_name])
            continue
             
        # DB 조회: 이미 아는 이름인가?
        cached_stage_name = db_manager.get_stage_name(raw_name)
        
        # 아티스트 활동명이 이미 있으면 그대로 사용
        if cached_stage_name:
            # DB에 있으면 메모리에도 등록 (다음 번에는 DB도 안 가도록)
            memory_cache[raw_name] = cached_stage_name 
            ...
            memory_cache[raw_name] = new_stage_name
...
def transform_event_data(start_date, end_date):
    ...
    memory_cache = {} # 이번 배치 실행 동안 사용할 임시 기억 장소
    ...
    try:
        ...
        for performance_detail in performance_detail_list:
            if performance_detail:
                ...
                cleaned_artist = process_artist_names(original_artist, title, db_manager, memory_cache)
