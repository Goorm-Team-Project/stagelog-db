...
def transform_event_data():
    performance_detail_list = get_performance_detail_list()
    transform_list = []
    try:
        for performence_detail in performance_detail_list:
            if(performence_detail):  
                ...
                transform_list.append({'kopis_id':performence_detail.get('mt20id'),
                    ...
                    'update_date':performence_detail.get('updatedate')[:19],
        ...
        print(json.dumps(transform_list, indent=4, ensure_ascii=False))
        print(f"kopis_transform 총{len(transform_list)}개의 데이터를 db필드용으로 변환")
...
def check_list_or_dict(relate_data):
    if isinstance(relate_data, list) and relate_data:
        relate_url = relate_data[0].get('relateurl', '리스트임')if relate_data else ''
        
    elif isinstance(relate_data, dict):
         relate_url = relate_data.get('relateurl','딕트임') 
